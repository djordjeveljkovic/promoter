<x-layouts.app :title="__('promoters.edit_form.page_title')">
    ...
    {{-- <div class="max-w-3xl mx-auto w-full rounded-lg bg-white p-6 shadow-sm dark:bg-zinc-800"> --}}
        <div class="flex items-center justify-between mb-6"> {{-- Added mb-6 for spacing similar to other pages --}}
            ...
            {{-- <a href="{{ route('admin.promoters.index') }}" class="text-sm text-indigo-600 hover:text-indigo-900 dark:text-indigo-400 dark:hover:text-indigo-300">&larr; Back to Promoters</a> --}}
        </div>

        <form method="POST" action="{{ route('admin.promoters.update', $promoter->id) }}" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="flex items-center justify-end space-x-3 pt-4">
                <a href="{{ route('admin.promoters.index') }}" {{-- Corrected route from 'promoters' to 'admin.promoters.index' for consistency --}}
