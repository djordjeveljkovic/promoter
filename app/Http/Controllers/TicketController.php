...
class TicketController extends Controller // Assuming this is the class name
{
    ...
    {
        $ticketTypes = TicketType::with('commissions')->latest()->paginate(10); // Eager load commissions, order by latest
        return view('pages.admin.ticket_type.index', compact('ticketTypes'));
    }
    ...
    {
        return view('pages.admin.ticket_type.create');
    }
    ...
    public function update(Request $request, $id) // Assumes Route Model Binding
    {
        ...
        try {
            ...
            DB::commit();
            return redirect()->route('ticket_type.index')->with('success', __('alert.ticket_type_updated_success'));
        } catch (\Exception $e) {
            DB::rollBack();
